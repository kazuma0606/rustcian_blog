...
v3 でコード基盤が完成し、v3.5 でコンテンツをプライベートリポジトリへ分離した。
v4 では Azure デプロイ前にフルスタックアプリケーションとして必要な機能を追加する。

### v3.5 を踏まえたアーキテクチャ上の前提

```
rustcian_blog_content (private)
  └→ push → build.yml → dist/ 生成 → デプロイ先へ配信

rustcian_blog (public)
  └→ 動的バックエンド（admin, API, 検索, コメント等）のみ
     content/ は起動時に git clone または mount
```

**バックエンドが責任を持つ範囲：** admin 操作・コメント・お問い合わせ・検索・AI メタデータ・監視

**バックエンドが関与しない範囲：** 記事の公開（= content repo への push）・静的 HTML 生成
...
| イベント | 担当 | 理由 |
|---|---|---|
| `PostPublished` | **content repo の build.yml** | 記事公開 = content repo push であり、バックエンドは関与しない |
| `StaticSiteRebuilt` | backend | `/admin/static/regenerate` ルートが起点 |
| `CommentReceived` | backend | コメント投稿 API が起点 |
| `ContactFormSubmitted` | backend | お問い合わせ API が起点 |
| `AiMetadataGenerated` | backend | AI メタデータ生成 API が起点 |
...
### content repo build.yml への PostPublished 通知追加

```yaml
# rustcian_blog_content/.github/workflows/build.yml に追加
- name: Notify Slack on publish
  ...
  run: |
    curl -s -X POST "$SLACK_WEBHOOK_URL" \
      ...
      --data '{"text":"✅ 静的サイトを更新しました（${{ github.sha }}）"}'
```
...
| 環境変数 | 説明 |
|---|---|
| `SLACK_WEBHOOK_URL` | backend 用。未設定時は Noop にフォールバック |
...
- サーバー起動時にインメモリ構築（`CONTENT_ROOT` から published 記事を読む）
...
v3.5 で静的サイト生成が CI 完結になったため、以下の方針とする。

| リソース | 方針 | 理由 |
|---|---|---|
| `azurerm_static_web_app` | **削除** | GitHub Pages または GitHub Actions から直接デプロイのため不要 |
| Blob コンテナ（静的アセット用） | **削除** | 静的アセットは content repo + CDN で管理 |
| Table（コメント・お問い合わせ用） | **存続** | バックエンドが引き続き利用 |
| その他 | **存続** | 変更なし |
...
└── modules/
    ...
    ├── storage/      ← Storage Account + Table（コメント等）のみ
...
| `azurerm_storage_account` | Table（コメント・お問い合わせ）|
...
### v3.5 を踏まえた BuildInfo の扱い

`_meta/build.json` は content repo の CI artifact として生成されるため、
本番バックエンドから直接参照できない。
代替として直近の `StaticSiteRebuilt` observability イベントの情報を表示する。
`BuildInfo` 型は削除し `LastBuildEvent` に置き換える。

### 追加 render 関数（frontend）

```rust
pub fn render_admin_dashboard(posts: Vec<PostSummary>) -> String
...
| `/admin/static` | 再生成ボタン（`/admin/static/regenerate` を呼ぶ） |
...
| 用途 | クレート / サービス |
|---|---|
| Slack 通知（backend） | `reqwest`（既存）|
| Slack 通知（CI） | `curl`（GitHub Actions step）|
| HTML サニタイズ | `ammonia` |
| 全文検索 | `tantivy` |
